...
import { promptApiKey, promptMaestro, promptPublicKey } from '../utils/prompts';
...
export class MaestroInitService {
    async run(workspacePath: string, vscode: any) {
        try {
            ...
            const publicKey = await promptPublicKey(vscode);
            const apiKey = await promptApiKey(vscode);
